...
export default {
  ...
  functional:true,
  render(h, ctx){
      var parentContent = ctx.props.icons.map((pro,index)=>{
          var Icon = h('span',{
             attrs:{
                 class:"fa "+pro
             }
        ...
        return h('div',{
            ...
            on:{
              click (e) {
                ...
                var targetTag = e.target.nodeName === "SPAN"?e.target.parentNode:e.target;
                targetTag.classList.add("active");
                const panels = document.querySelectorAll("#"+ctx.props.id+" .panel");
                panels.forEach(p=>{
                  p.setAttribute('style',hidePanel);
                })
                const selectId = document.querySelector("#"+ctx.props.id+" #"+targetTag.id);
